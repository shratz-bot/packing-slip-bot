...
uploads them to Google Drive, transfers ownership to the Drive owner account,
and writes the file URL back to the sheet.

Requirements:
    pip install gspread google-auth google-api-python-client weasyprint pydyf
...
Environment variables (set as GitHub Secrets):
    ...
    DRIVE_OWNER_EMAIL            - Your personal Gmail address (receives file ownership)
    SHEET_NAME                   - Sheet tab name (optional, defaults to "Orders")
"""
...
SHEET_NAME        = os.environ.get("SHEET_NAME", "Orders")
...
DRIVE_OWNER_EMAIL = os.environ["DRIVE_OWNER_EMAIL"]
...
def fetch_image_as_b64(drive_service, file_id):
...
def upload_pdf(drive_service, pdf_bytes, filename, folder_id):
    media = MediaIoBaseUpload(io.BytesIO(pdf_bytes), mimetype="application/pdf", resumable=False)
    file_meta = {"name": filename, "parents": [folder_id], "mimeType": "application/pdf"}
    ...
    # Transfer ownership to your personal Google account so the file uses
    # your storage quota, not the service account's (which has none).
    try:
        ...
        log.info(f"  Ownership transferred to {DRIVE_OWNER_EMAIL}")
    except Exception as e:
        log.warning(f"  Could not transfer ownership (non-fatal): {e}")
...
def build_html(order, ebay_b64, shratz_b64, qr_b64):
...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  body {{ font-family: Arial, Helvetica, sans-serif; font-size: 13px; color: #111; padding: 36px 40px; width: 750px; }}
  .slip-top {{ display: flex; justify-content: space-between; align-items: center; padding-bottom: 14px; border-bottom: 1.5px solid #111; margin-bottom: 20px; }}
  ...
  .addr-row {{ display: flex; margin-bottom: 16px; }}
  .addr-block {{ flex: 1; margin-right: 40px; }}
  ...
  th {{ font-size: 11px; font-weight: bold; padding: 6px 0; border-bottom: 1px solid #111; text-align: left; }}
  ...
  .bottom {{ display: flex; align-items: center; margin-top: 8px; }}
  .msg {{ flex: 1; font-size: 12px; line-height: 1.7; margin-right: 24px; }}
  .msg-title {{ font-weight: bold; margin-bottom: 4px; font-size: 12px; }}
  .qr {{ display: flex; flex-direction: column; align-items: center; margin-right: 24px; flex-shrink: 0; }}
  .qr-txt {{ font-size: 11px; font-weight: bold; text-align: center; margin-bottom: 4px; }}
  .qr-txt-bottom {{ font-size: 11px; font-weight: bold; text-align: center; margin-top: 4px; }}
  ...
  .trow.grand {{ font-weight: bold; border-top: 1px solid #ccc; margin-top: 4px; padding-top: 5px; }}
  .footnote {{ font-size: 11px; color: #555; margin-top: 16px; border-top: 0.5px solid #ddd; padding-top: 10px; line-height: 1.6; }}
</style>
</head>
<body>

<div class="slip-top">
  <div class="top-left"><img class="ebay-logo" src="data:image/png;base64,{ebay_b64}" alt="eBay"></div>
  <div class="top-center"><img class="shratz-logo" src="data:image/jpeg;base64,{shratz_b64}" alt="Shratz105"></div>
  <div class="top-right"><div class="ps-label">PACKING SLIP</div></div>
</div>

<div class="addr-row">
  <div class="addr-block">
    <div class="lbl">Ship to</div>
    <p><strong>{order["buyer_name"]}</strong><br>{order["addr_line1"]}<br>{order["addr_city"]}, {order["addr_state"]}, {order["addr_zip"]}<br>{order["addr_country"]}</p>
  </div>
  <div class="addr-block">
    <div class="lbl">Ship from</div>
    <p><strong>{SELLER["name"]}</strong><br>{SELLER["line1"]}<br>{SELLER["city"]}, {SELLER["state"]}, {SELLER["zip"]}<br>{SELLER["country"]}</p>
  </div>
</div>
...
<table>
  ...
  <tbody>
    <tr>
      <td>{order["item_title"]}<br><span class="item-id">(Item ID: {order["item_id"]})</span></td>
...
<div class="bottom">
  ...
  <div class="qr">
    ...
    <div class="qr-txt-bottom">Get a $15 credit!</div>
  </div>
  <div class="totals">
...
<div class="footnote">
  **Order total includes eBay collected tax. eBay collects and remits the tax to the tax authorities in accordance with applicable state law.
</div>
    ...
    for i, row in enumerate(data_rows, start=2):
        ...
        try:
            order = {
                ...
                "item_total":       fmt_currency(float(cell(row, "ITEM_PRICE") or 0) * int(cell(row, "QUANTITY") or 1)),
            ...
            filename  = f"PackingSlip_{order['order_id']}_{date_str}.pdf"
